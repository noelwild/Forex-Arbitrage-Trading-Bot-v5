...
function App() {
  ...
  const closePosition = async (positionId) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(`Position closed! P&L: ${formatCurrency(data.realized_pnl)}`);
        if (config) {
          loadPositions(config.id);
          loadPerformance(config.id);
        }
  ...
  const hedgePosition = async (positionId) => {
    if (!window.confirm('Are you sure you want to hedge this position?')) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        alert(`Position hedged successfully!`);
        if (config) {
          loadPositions(config.id);
        }
      } else {
        alert(`Failed to hedge position: ${data.detail}`);
      }
    } catch (error) {
      console.error('Error hedging position:', error);
      alert('Error hedging position');
    }
  };
