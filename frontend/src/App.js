...
function App() {
  ...
  const filteredOpportunities = opportunities.filter(opp => {
    return (
      (!pairFilter || opp.currency_pairs.some(pair => pair.toLowerCase().includes(pairFilter.toLowerCase()))) &&
      (!brokerFilter || opp.brokers.some(broker => broker.toLowerCase().includes(brokerFilter.toLowerCase()))) &&
  ...
  const filteredTrades = trades.filter(trade => {
    const tradeDate = new Date(trade.execution_time);
    const filterDate = dateFilter ? new Date(dateFilter) : null;
    
    return (
      (!pairFilter || trade.currency_pairs.some(pair => pair.toLowerCase().includes(pairFilter.toLowerCase()))) &&
      (!brokerFilter || trade.broker.toLowerCase().includes(brokerFilter.toLowerCase())) &&
      (!typeFilter || trade.type === typeFilter) &&
      (!dateFilter || (filterDate && tradeDate.toDateString() === filterDate.toDateString()))
    );
  });
  ...
  const getUniquePairs = () => {
    const pairs = new Set();
    opportunities.forEach(opp => opp.currency_pairs.forEach(pair => pairs.add(pair)));
    trades.forEach(trade => trade.currency_pairs.forEach(pair => pairs.add(pair)));
    return Array.from(pairs).sort();
  };

  const getUniqueBrokers = () => {
    const brokers = new Set();
    opportunities.forEach(opp => opp.brokers.forEach(broker => brokers.add(broker)));
    trades.forEach(trade => brokers.add(trade.broker));
    Object.keys(marketData).forEach(broker => brokers.add(broker));
    return Array.from(brokers).sort();
  };

  const FilterBar = ({ showDateFilter = false, showTypeFilter = true }) => (
    <div className="bg-gray-50 p-4 rounded-lg mb-4">
      ...
      <div className="mt-2 flex justify-between items-center">
        <span className="text-sm text-gray-600">
          Showing {showDateFilter ? filteredTrades.length : filteredOpportunities.length} results
        </span>
        <button
          onClick={() => {
            setPairFilter('');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="mt-8 p-4 bg-blue-50 rounded-lg">
          <h3 className="text-lg font-semibold text-blue-900 mb-3">📝 Broker Setup Instructions</h3>
          <div className="space-y-3 text-sm text-blue-800">
            <div>
              <strong>OANDA:</strong> Get API key from <a href="https://developer.oanda.com/" target="_blank" rel="noopener noreferrer" className="underline">OANDA Developer Portal</a>
            </div>
            <div>
              <strong>Interactive Brokers:</strong> Enable API in TWS/IB Gateway, use localhost:7497 for paper trading
            </div>
            <div>
              <strong>FXCM:</strong> Register for API access at FXCM Developer Portal
            </div>
            <div>
              <strong>XM/MetaTrader:</strong> Use your existing MT5 account credentials
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 p-4 bg-yellow-50 rounded-lg">
          ...
          <div className="space-y-2 text-sm text-yellow-800">
            <p>• For testing purposes, you can input fake credentials containing "fake", "test", or "invalid"</p>
            <p>• These will always show connection failures with appropriate error messages</p>
            <p>• Real credentials will be validated against actual broker APIs</p>
            <p>• All credentials are encrypted and stored securely in the database</p>
          </div>
        </div>
      </div>
    </div>
